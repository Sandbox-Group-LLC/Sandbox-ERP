...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, size, contentType, uploadType } = body
    ...
    // Return storage path for permanent storage (not signed URL that expires)
    return NextResponse.json({
      uploadUrl,
      fileUrl: `storage://${fullPath}`,
      storagePath: fullPath,
      metadata: { name, size, contentType, uploadType },
    })
  } catch (error) {
